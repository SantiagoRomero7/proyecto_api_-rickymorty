...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("btn-characters")
    ?.addEventListener("click", () => showSection("characters"));
  document
    .getElementById("btn-episodes")
    ?.addEventListener("click", () => showSection("episodes"));
  document
    .getElementById("btn-locations")
    ?.addEventListener("click", () => showSection("locations"));
});
...
function displayEpisodes(episodes) {
  ...
  episodes.forEach((ep) => {
    ...
    card.classList.add("card");
    card.innerHTML = `
        <img src="https://via.placeholder.com/300x150?text=${encodeURIComponent(
          ep.name
        )}" alt="${ep.name}" />
        <div class="card-content">
          <h3>${ep.name}</h3>
